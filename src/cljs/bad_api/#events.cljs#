...
(defn api-get-products [product]
    (go (let [response (<! (http/get (str btr/proxy "https://bad-api-assignment.reaktor.com/v2/products/" product)
                                     {:with-credentials? false}))]
  ...
  (rf/reg-event-fx 
   :init-load
   (fn [_ [_ ]]
     (doseq [product btr/products-list]
     (api-get-products product))
     ))
